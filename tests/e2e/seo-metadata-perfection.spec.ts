...
test.describe('SEO & Metadata Perfection', () => {
  ...
  test('should render project-specific metadata and JSON-LD', async ({
    page,
  }) => {
  ...
  test('should update canonical and hreflang on language switch', async ({
    page,
  }) => {
    ...
    await expect(page.locator('link[rel="canonical"]')).toHaveAttribute(
      'href',
      /.*\/en\/work\/atdd-magazine-test/
    );

    // WHEN: Switch to Thai
    const thButton = page
      .locator('[data-testid="language-switcher-th"]:visible')
      .first();
    ...
    await expect(page.locator('link[rel="canonical"]')).toHaveAttribute(
      'href',
      /.*\/th\/work\/atdd-magazine-test/
    );

    // AND: Hreflang alternates are correct
    await expect(
      page.locator('link[rel="alternate"][hreflang="en"]')
    ).toHaveAttribute('href', /.*\/en\/work\/atdd-magazine-test/);
    await expect(
      page.locator('link[rel="alternate"][hreflang="th"]')
    ).toHaveAttribute('href', /.*\/th\/work\/atdd-magazine-test/);
  });
});
