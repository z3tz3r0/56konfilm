...
test.describe('Project Magazine Layout', () => {
  ...
  test('should render magazine blocks and new metadata', async ({
    page,
    setMode,
  }) => {
    ...
    await expect(
      page.getByTestId('project-client'),
      'Client name should be visible'
    ).toBeVisible();
    await expect(
      page.getByTestId('project-year'),
      'Project year should be visible'
    ).toBeVisible();
    await expect(
      page.getByTestId('project-services'),
    ...
    await expect(
      page.locator('section[data-sanity-type="heroSection"]'),
      'Hero Section block should be rendered'
    ).toBeVisible();

    // 2. Two Column Section (Text/Image)
    await expect(
      page.locator('section[data-sanity-type="twoColumnSection"]'),
      'Two Column Section block should be rendered'
    ).toBeVisible();

    // 3. Media Gallery Section (Grid)
    await expect(
      page.locator('section[data-sanity-type="mediaGallerySection"]'),
      'Media Gallery Section block should be rendered'
    ).toBeVisible();
  });
});
