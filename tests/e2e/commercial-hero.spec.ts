...
test.describe('Commercial Homepage - Brutal Loop', () => {
  ...
  test('should play muted video loop in Commercial Mode', async ({
    page,
    setMode,
  }) => {
    ...
    const poster = page
      .getByAltText('Section background')
      .or(page.getByAltText('Video poster'));
    ...
    await expect(async () => {
      const paused = await video.evaluate(
        (vid) => (vid as HTMLVideoElement).paused
      );
      expect(paused).toBe(false);
    }).toPass({ timeout: 5000 });
  });

  test('should display "WE SHOOT HARD" text overlaid', async ({
    page,
    setMode,
  }) => {
    await setMode('production');
  ...
  test('should react to scroll with velocity-based skew', async ({
    page,
    setMode,
    browserName,
  }) => {
    ...
    const initialTransform = await heroText.evaluate(
    ...
    const transformedStyle = await heroText.evaluate(
      (el) => window.getComputedStyle(el).transform
    );
