...
test.describe('Wedding Homepage Sections', () => {
  ...
  test('should render Packages Section with 3 distinct tiers', async ({
    page,
  }) => {
  ...
  test('should navigate Testimonials Carousel using arrows', async ({
    page,
  }) => {
    ...
    const firstTestimonial = section
      .getByTestId('testimonial-item')
      .filter({ visible: true });
    ...
    const nextTestimonial = section
      .getByTestId('testimonial-item')
      .filter({ visible: true });
    await expect(nextTestimonial).not.toHaveText(initialText);
  });

  test('should display Philosophy Section with Serif typography', async ({
    page,
  }) => {
  ...
  test('should use Wedding mode color palette consistently', async ({
    page,
  }) => {
    ...
    await expect(packages).toHaveCSS(
      'background-color',
      /rgba?\(250, 247, 242/
    );
    await expect(testimonials).toHaveCSS(
      'background-color',
      /rgba?\(250, 247, 242/
    );
    await expect(philosophy).toHaveCSS(
      'background-color',
      /rgba?\(250, 247, 242/
    );
  });
