...
We will leverage `shadcn/ui` _only_ for complex accessible primitives. We will strictly strip their default styles and apply our `Dual-Mode` classes. All shadcn components must accept a `mode` prop or read from the `ModeProvider`.
